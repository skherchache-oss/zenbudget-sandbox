...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLocalMode }) => {
  ...
  return (
    <div className="min-h-[100dvh] w-full flex flex-col bg-slate-950 font-sans bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-black">
      
      {/* Conteneur principal qui s'adapte à l'écran */}
      <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8">
        
        <div className="w-full max-w-[440px] bg-white/95 backdrop-blur-2xl p-8 sm:p-12 rounded-[40px] sm:rounded-[50px] shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-white/20 flex flex-col">
          
          <div className="w-16 h-16 sm:w-20 sm:h-20 bg-slate-900 rounded-[24px] sm:rounded-[28px] shadow-2xl flex items-center justify-center mb-6 sm:mb-8 mx-auto transform -rotate-6 shrink-0">
            <IconLogo className="w-10 h-10 sm:w-12 sm:h-12 text-white" />
          </div>
          ...
          <p className="text-slate-500 mb-8 sm:mb-10 text-sm font-medium leading-relaxed px-2 sm:px-4 text-center">
          ...
          <button 
            type="button"
            onClick={onLocalMode} 
            className="mt-8 sm:mt-10 w-full py-4 bg-slate-900 text-white rounded-[20px] text-[10px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-slate-800 active:scale-95 transition-all"
          >
            Continuer en Mode Invité
          </button>
        </div>
        
        {/* Texte en bas intégré dans le scroll pour éviter les chevauchements */}
        <p className="mt-8 mb-4 text-slate-600 text-[10px] font-bold uppercase tracking-[0.3em] opacity-40">
          ZenBudget — Est. 2026
        </p>
      </div>
    </div>
